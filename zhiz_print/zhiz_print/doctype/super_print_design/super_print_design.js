...
class SuperPrintDesigner {
    ...
    showPageProperties() {
        ...
        const _row = (id, label, val, aid, aval) => '<div style="display:flex;gap:4px;align-items:center"><select id="' + aid + '" class="form-control input-sm" style="width:50px !important;font-size:10px;padding:2px;flex-shrink:0"><option value="Left"' + (aval==='Left'?' selected':'') + '>L</option><option value="Center"' + (aval==='Center'?' selected':'') + '>C</option><option value="Right"' + (aval==='Right'?' selected':'') + '>R</option></select><textarea id="' + id + '" class="form-control input-sm" rows="1" placeholder="' + label + '" style="flex:1;width:auto !important;font-size:11px;min-width:0">' + (val || '') + '</textarea></div>';
