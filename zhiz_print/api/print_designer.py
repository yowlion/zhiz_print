...
def get_available_designs(doctype, docname=None):
    ...
    designs = frappe.get_all(
        ...
        fields=["name", "design_name", "print_paper", "priority", "draft_no_print", "page_count"],
    ...
    if cells:
        for cell in cells:
            ...
            doc.append("design_items", {
                ...
                "barcode_show_text": cint(cell.get("barcode_show_text", 1)),
                "barcode_text_size": cint(cell.get("barcode_text_size", 10)) or 10,
...
def load_design_data(design_name):
    ...
    if doc.design_items:
        for item in doc.design_items:
            ...
            cell_data = {
                ...
                "barcode_text_size": item.barcode_text_size or 10,
    ...
    for item in (getattr(doc, 'design_seals', None) or []):
        ...
        seals.append({
            ...
            "pos_x": item.pos_x,
            "pos_y": item.pos_y,
            "position_mode": item.position_mode or "paper",
            "offset_x": item.offset_x if item.offset_x is not None else 0,
            "offset_y": item.offset_y if item.offset_y is not None else 0,
