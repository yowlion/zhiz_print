...
![设计器](docs/screenshots/designer.png)
...
### 数据源侧栏·拖拽绑定 (v15.22.36+ 新)

设计器左侧 **180px 可折叠侧栏**,树状展示当前设计的五类可用数据源,是**占位符的标准录入入口**:

| 分组 | 内容 |
|---|---|
| 单据字段 | `doc` 主表字段(收纳进「**主表**」虚拟组)+ 各**子表**字段(组与组平级) |
| 报表字段 | `rep` 报表字段/筛选/数据列(报表模式) |
| 设计参数 | `param` 已定义参数 |
| 数据查询 | 各 query 定义的返回列 |

- **拖拽叶子节点到单元格** → 自动生成对应占位符(如拖入 item_code 生成 `{doc.items.item_code}`);**双击**叶子兜底写入(或复制)
- 字段 label 经翻译显示(如「物料名称(item_code)」),按当前语言
- 默认展开到二级:顶层单据字段 + 主表/子表组标题可见、叶子收起,首屏有结构感;点击分组折叠/展开,折叠后缩为 28px 图标竖条
- 打开设计器即加载;查询/参数增删改、保存、目标单据切换后自动重载
- 写入后右侧属性面板「值」输入框实时同步
...
### fx 方法助手·= 表达式输入辅助 (v15.22.44+ 新)

单元格值以 `=` 开头时,右侧属性面板「值」输入框旁出现 **fx 按钮**,点击(或键入 `=` 自动)弹出**方法选择器**:

- **只提示函数方法**:合计函数(`rowsum` / `row`)+ 求值函数(`get_value` / `max` / `min` / `round` 独立三项)+ `doc` 引导项(目标单据对象取字段);**占位符不入列** —— 占位符的标准入口是数据源侧栏拖拽(见上)
...
**float / int 求值结果**用 `_fmt_val` 格式化(去尾零:`50.0 → "50"`、`5.10 → "5.1"`);**表达式显式返回字符串则原样显示** —— `fmt(x, 2)` 的定长精度(如 `12.00`)不会被剥尾零。字段为空导致公式非法时(如 `100*`)记录错误日志并显示原式,便于发现缺数据。
...
| html | **网页代码**(v15.22.53 新):值按 HTML 文档解析渲染 | — |

### 网页代码(html,v15.22.53 新)

单元格值按**完整 HTML 文档**解析渲染,适用于在打印模板中嵌入第三方面单/凭证页(如顺丰运单的 `print_html` 字段):

- **JSON 数组自动解包**:值为 `[{"waybill_no": "...", "html": "<!DOCTYPE..."}]` 形态时逐元素取 `html` 键,多包裹纵向堆叠;纯 HTML 字符串直接渲染
- **iframe 隔离**:文档经 `sandbox` iframe 渲染,自带的全局样式(`*{margin:0}`、`@page` 等)不会污染打印页
- **1:1 基准等比缩放**:按文档声明的物理尺寸(mm)为基准,等比缩放到单元格可用区域并居中 —— 单元格尺寸调整,显示跟着调整
- **PDF 兼容**:wkhtmltopdf / WeasyPrint 两条引擎路径在生成前自动展开为内联容器(CSS 加作用域前缀隔离),打印/预览/PDF 三路一致
...
![打印预览](docs/screenshots/print-preview.png)
...
![打印日志](docs/screenshots/print-log.png)
...
![模板平台](docs/screenshots/template-store.png)
...
平台对非作者浏览者做多层脱敏,**中心服务器存原版、返回前脱敏**(v15.20+ 服务端执行,老版本客户端同样受保护):

| 层 | 规则 |
|---|---|
| **单元格脱敏**(v15.22.55+ 新) | 作者标记的单元格,在他人视角的**打印效果**中逐字符显示为 `*`(空白保留维持排版);网页代码单元格(iframe 面单等)的属性内容同规则星化 |
| **公司名称** | 替换为固定占位 |
| **二维码/URL** | 真实客户域名(含端口)替换为 `www.xxx.com`(保留路径与 `{doc.name}`);已渲染成图片的二维码按模块矩阵匹配定位后重新生成脱敏码(此项由新版客户端执行) |

**单元格脱敏设置**(v15.22.55+ 新):

1. 打开**自己的模板**(卡片带「我的」徽标;同公司名的多台服务器视为同一作者,任一台新版机器均可设置)→ 切到「**设计效果**」页签
2. **点击单元格切换脱敏**(黄色高亮 = 已标记)→ 点「**保存脱敏设置**」
3. 效果:作者本人看打印效果仍是真实数据;其他公司的浏览者看到标记格为 `***`

**规则**:

- 只脱敏「打印效果」;「设计效果」是占位符结构(本就是给别人看字段设置的),不做脱敏
- 脱敏在**中心服务器返回前**执行 —— 老版本 zhiz_print 客户端同样受保护
- **中心服务器本机**视角永远白名单不脱敏(平台调试管理)
- 重推更新模板**不会清空**已保存的脱敏标记
- **老模板自动升级**:无单元格标识的历史模板(含老版本客户端上传的),中心返回时按表格网格算法动态注入标识,立即支持点选设置,无需重新分享
- 图片类内容(条码/二维码图片内的编码数据)不在文本脱敏范围
