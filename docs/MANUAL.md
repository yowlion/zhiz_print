...
![Zprint Setting](images/03_zprint_setting.png)
*图 2-1  全局设置 Zprint Setting(启用开关 / PDF 引擎选择)*
...
![设计器主界面](images/01_designer_main.png)
*图 3-1  设计器主界面 — 网格、工具栏、单元格内容(占位符 / 表达式 / rowsum 全部可见)*
...
- **Settings(设置)** — 启用条件 / 草稿拦截 / 优先级 / 纸张 等(见图 10-1)
...
![行属性面板-数据排序](images/07_row_panel.png)
*图 6-1  行属性面板 — 选中第5行(数据驱动行),「数据排序」填 `row.2 ASC` 按第2列排序*
...
![Settings tab](images/06_settings_tab.png)
*图 10-1  Settings tab — 启用条件、草稿拦截、优先级、目标单据、纸张*
...
![Zprint Setting](images/03_zprint_setting.png)
*图 14-1  Zprint Setting — 全局开关与 PDF 引擎*
