...
- **Datasource Sidebar & Drag-Drop** — Tree sidebar in the designer (doc/main-table/child-table/rep/param/query fields); drag a field onto a cell to insert its placeholder, translated labels, auto-reload
- **fx Method Assistant** — Typing `=` in a cell value pops a method picker (aggregate + eval functions with usage & examples, live filtering)
- **HTML Cell Type** — Render a full HTML document inside a cell (e.g. courier waybill print_html): JSON auto-unwrap, sandboxed iframe isolation, 1:1 mm-based scaling, PDF-engine expansion
